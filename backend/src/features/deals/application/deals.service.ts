...
export class DealsService {
  ...
  canSee(user: UserEntity, deal: DealEntity) {
    return user.role === UserRole.Admin || deal.ownerId === user.id || deal.createdBy === user.id;
  }

  async list(user: UserEntity) {
    if (user.role === UserRole.Admin) return this.deals.find({ order: { id: 'ASC' } });
    return this.deals.createQueryBuilder('deal')
      .where('deal.ownerId = :id OR deal.createdBy = :id', { id: user.id })
      .orderBy('deal.id', 'ASC')
      .getMany();
  }
  ...
  async update(id: number, body: any, user: UserEntity) {
    ...
    if (body.ownerId !== undefined && user.role === UserRole.Admin) deal.ownerId = Number(body.ownerId);
    return this.deals.save(deal);
  }
